...
console.log('Using credentials from production.env:');
...
async function testAuth() {
  try {
    const res = await fetch(baseUrl, {
      ...
      body: JSON.stringify({
        order_id: `test_prod_auth_${Date.now()}`,
        order_amount: 1.00,
        order_currency: 'INR',
        customer_details: {
          customer_id: 'test_prod_user',
    ...
    if (res.ok) {
      console.log('\nSUCCESS! Production credentials work perfectly.');
    } else {
